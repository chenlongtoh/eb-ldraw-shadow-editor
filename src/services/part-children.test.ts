...
import { listDirectChildFiles } from './part-children'

describe('listDirectChildFiles', () => {
  it('lists unique direct type-1 children and keeps original spelling', () => {
    const content = [
      '0 Brick 2 x 2',
    ...
    expect(listDirectChildFiles(content)).toEqual([
  ...
  it('returns an empty list when there are no type-1 children', () => {
    expect(listDirectChildFiles('0 Comment\n4 16 0 0 0 1 1 1 2 2 2 3 3 3\n')).toEqual([])
  })
})
